...
RC RecordPageHandler::init(DiskBufferPool &buffer_pool, PageNum page_num, ReadWriteMode readonly)
{
  ...
  if (readonly==ReadWriteMode::READ_ONLY) {
    frame_->read_latch();
  } else {
   frame_->write_latch();
  }
...
RC RecordFileHandler::update_record(int offset, int index, Value &value, const Record &record) {
  ...
  if ((rc = page_handler.init(*disk_buffer_pool_, record.rid().page_num, ReadWriteMode::READ_WRITE)) != RC::SUCCESS) {
...
RC RecordFileHandler::update_record(Record *rec)
{
  ...
  ret = record_page_handler.init(*disk_buffer_pool_, rec->rid().page_num, ReadWriteMode::READ_WRITE /*readonly*/);
  if (ret != RC::SUCCESS) {
