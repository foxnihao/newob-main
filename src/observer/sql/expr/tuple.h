...
virtual ~RowTuple()
{
  for (FieldExpr *&spec : speces_) {  // 使用引用来修改指针本身
    delete spec;
    spec = nullptr;  // 删除后将指针置为 nullptr，避免悬挂指针
  }
  speces_.clear();  // 清空指针数组
}
...
public:
  ...
  void set_names(const std::vector<TupleCellSpec> &specs) { cellSpecs_ = specs; }
  ...
  static RC make(const Tuple &tuple, ValueListTuple &value_list)
  {
    ...
    for (int i = 0; i < cell_num; i++) {
      ...
      value_list.cellSpecs_.push_back(spec);
    }
    return RC::SUCCESS;
  }
