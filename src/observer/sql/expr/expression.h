...
public:
  ...
  {
    return new StarExpr(table_name_.c_str());
  }
...
public:
  ...
  Expression *clone() const override
  {
    return new UnboundFieldExpr(table_name_, field_name_);
  }
...
public:
  ...
  std::unique_ptr<Expression> &child() { return child_; }
    Expression *clone() const override
  {
    return new CastExpr(std::unique_ptr<Expression>(child_->clone()), cast_type_);
  }
...
Expression* clone() const override {
  return new ComparisonExpr(comp_, std::unique_ptr<Expression>(left_->clone()), std::unique_ptr<Expression>(right_->clone()));
}
...
Expression* clone() const override {
  ...
  for (const auto &child : children_) {
    cloned_children.push_back(std::unique_ptr<Expression>(child->clone()));
  }
  return new ConjunctionExpr(conjunction_type_, cloned_children);
}
...
public:
  ...
  std::unique_ptr<Expression> &right() { return right_; }
Expression*clone() const override {
  return new ArithmeticExpr(arithmetic_type_, std::unique_ptr<Expression>(left_->clone()), std::unique_ptr<Expression>(right_ ? right_->clone() : nullptr));
}
...
Expression* clone() const override {
  return new UnboundAggregateExpr(aggregate_name_.c_str(), child_ ? child_->clone() : nullptr);
}
...
Expression* clone() const override {
  return new AggregateExpr(aggregate_type_, std::unique_ptr<Expression>(child_->clone()));
}
