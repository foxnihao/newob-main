...
static void wildcard_fields(Table *table, vector<unique_ptr<Expression>> &expressions, bool is_multi_table)
{
  const TableMeta &table_meta = table->table_meta();
  const int        field_num  = table_meta.field_num();
  for (int i = table_meta.sys_field_num(); i < field_num; i++) {
    ...
    // 如果是多表查询，给字段名添加表名前缀
    if (is_multi_table) {
      std::string prefixed_name = std::string(table->name()) + "." + field.field_name();
      field_expr->set_name(prefixed_name.c_str());
    } else {
      field_expr->set_name(field.field_name());  // 单表时保持原样
    }
    ...
    unique_ptr<Expression> &expr, vector<unique_ptr<Expression>> &bound_expressions)
{
  ...
  bool is_multi_table = (tables_to_wildcard.size() > 1);  // 判断是否是多表查询

  for (Table *table : tables_to_wildcard) {
    wildcard_fields(table, bound_expressions, is_multi_table);
  }
